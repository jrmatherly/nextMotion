...
class Logger {
  ...
  private constructor(module?: LogModule) {
    // Convert log level string to number for consola
    ...
    const logLevel = env.NEXT_PUBLIC_LOG_LEVEL 
      ? logLevelMap[env.NEXT_PUBLIC_LOG_LEVEL] || 3 // Default to info (3) if invalid
      : showLogger ? 4 : 2; // Debug if showLogger is true, warn otherwise
